/*
 * Verify unresolved local and alias imports in website source.
 * Usage: node scripts/checkLocalImports.js
 */
...
const TARGET_DIRS = [SRC_ROOT, path.join(ROOT, 'scripts')];
...
for (const file of files) {
  ...
  for (const spec of specs) {
    const result = resolveSpec(spec, file);
...
if (failures.length === 0) {
  console.log('OK: no unresolved/case-mismatched local imports in website source.');
  process.exit(0);
}
